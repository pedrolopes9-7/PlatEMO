function KEY = double2hash32bit(Individual)
    ...
    hashOfHash = zeros(1,length(Individual));
    strResult = "";
    hash = 0;
    
    for j=1:length(hashOfHash)
        chunk = num2str(Individual(j));
        result = mod(sum(chunk), 256);
        
        for i = 1:size(Individual)
            tableIndex = bitxor(hash, result);
            if tableIndex==0
            ...
            hash = lookUpTable(tableIndex);
        end
        hashOfHash(j) = hash;
    end

    for i=1:length(hashOfHash)   
        strAux = string(hashOfHash(i));
        strResult = strResult + strAux;
    end
    
    KEY = uint64(str2double(strResult));
end
